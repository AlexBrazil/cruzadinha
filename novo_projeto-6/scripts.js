...
function resizeGridShell() {
  ...
  const availableWidth = shellRect.width - paddingX;
  if (availableWidth <= 0) {
    return;
  }

  const currentCell = rect.width;
  const maxCell = Math.min(64, currentCell * 1.1);
  const minCell = 32;
  const computedCell = Math.max(
    minCell,
    Math.min(
      maxCell,
      (availableWidth - gap * Math.max(0, cols - 1)) / cols
    )
  );

  const gridHeight = computedCell * rows + Math.max(0, rows - 1) * gap;

  elements.grid.style.setProperty("--cell-size", `${computedCell}px`);
  elements.gridShell.style.height = `${Math.ceil(gridHeight + paddingY)}px`;
}
