...
const elements = {
  ...
  hiddenInput: document.getElementById("mobile-input")
};
...
function attachEvents() {
  ...
  if (elements.hiddenInput) {
    elements.hiddenInput.addEventListener("input", handleHiddenInput);
    elements.hiddenInput.addEventListener("keydown", handleHiddenKeyDown);
  }
}
...
function handleKeyDown(event) {
  if (state.isTouch && event.target === elements.hiddenInput) {
    return;
  }
  ...
  const key = event.key;
  if (key.length === 1 && /[A-Za-zÀ-ÖØ-öø-ÿ0-9]/.test(key)) {
...
function setCellValue(cell, value) {
  ...
  cell.userChar = value ? value.toUpperCase() : "";
  cell.normalizedChar = normalized;
  updateCellDisplay(cell);
  if (state.isTouch && elements.hiddenInput && cell === state.activeCell) {
    elements.hiddenInput.value = cell.userChar || "";
    requestAnimationFrame(() => {
      const length = elements.hiddenInput.value.length;
      elements.hiddenInput.setSelectionRange(length, length);
    });
  }
...
function focusHiddenInput(cell) {
  if (!state.isTouch || !elements.hiddenInput) {
    return;
  }
  elements.hiddenInput.value = cell?.userChar || "";
  requestAnimationFrame(() => {
    elements.hiddenInput.focus({ preventScroll: true });
    const length = elements.hiddenInput.value.length;
    elements.hiddenInput.setSelectionRange(length, length);
  });
}
...
function handleHiddenInput(event) {
  if (!state.isTouch || !state.activeCell) {
    event.target.value = "";
    return;
  }
  ...
  setCellValue(state.activeCell, char);
  event.target.value = "";
  moveToNextCell();
}

function handleHiddenKeyDown(event) {
  if (!state.isTouch || !state.activeCell) {
    return;
  }
